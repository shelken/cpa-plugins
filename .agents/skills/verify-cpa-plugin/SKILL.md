...
下文用 slug 指代路径：不带前缀的路径相对本仓根目录，`{ref-dir}` 指固定的临时克隆目录（惯例 `/tmp/ref`），`{tmp}` 指临时工作目录，`{pi-codebuddy-provider}` 指另一个仓库。
...
| 可视配置字段 | `v7.2.158` 起 | `internal/api/handlers/management/plugins.go` 把 `Metadata.ConfigFields` 透给管理面，nil 时置空数组。158 与 159 同构 |
| 额度 | `v7.2.159` | `quota.*` 方法在该版引入。同一产物在 `v7.2.158` 上 `supports_quota` 为 `null`，`quota/providers` 返回 404 |

版本与能力都从管理面读：响应头 `X-CPA-VERSION` 给版本，`plugins` 条目的能力字段给能力，别从部署仓库的镜像 tag 或版本号推断。
...
2. **账号可用性的判断。** 账号被风控或限额时，agent 无法从错误码区分「账号问题」与「插件缺陷」。凭据档出现的错误一律先标注账号状态，不要在账号未验证时定性（教训见 `postmortems/003-unusable-account-verification.md`）
3. **真实环境的主观判定。** 用户在自己环境引入后的体验、上游策略变化要不要跟进，属于用户判断，不要代替他下结论

本仓没有可测账号。凭据档必须等用户现场扫码登录或提供凭据，在此之前只跑前三档；用户环境里的账号以他现场确认为准，不要拿旧凭据反复试。

## Launch

```bash
# 沙箱: 构建插件 -> 按平台放置 -> 生成配置 -> 启动宿主 -> 三条断言
go run scripts/dev-sandbox.go -plugin workbuddy -host-src {ref-dir}/CLIProxyAPI -timeout 120s
```

就绪判据是脚本自己打的 `[+] 沙箱验证通过` 与三条 `断言通过`，端口默认 `18317`。没有这三行就是没起来，别接着往下驱动。

宿主的解析顺序是 `-host <二进制>` > `$CPA_HOST_BIN` > `~/.cache/cpa-plugins/host/v<SDK版本>/cliproxyapi` > `-host-src <源码目录>`（现场构建并落缓存）。源码目录必须先 `git checkout` 到插件 `go.mod` 指定的那个 tag，脚本会核对，不一致直接失败。改动频繁时用 `{ref-dir}/CLIProxyAPI` 这个固定克隆，别反复下载。
...
**先确认可达性与目标。** 401 表示网络通、只差密钥；超时才是网络不通。两者要分开说，别混成一句「连不上」。

**版本从响应头读。** 每次管理面响应都带 `X-CPA-VERSION`、`X-CPA-COMMIT`、`X-CPA-BUILD-DATE`、`X-CPA-SUPPORT-PLUGIN`，它们在鉴权之前写入，所以密钥不对时也拿得到。部署仓库的镜像 tag 与 Pod 实际运行的版本可能不一致。

**鉴权统一走 `scripts/management-api.go`，不要手搓 curl。** 密钥来源、请求头形式、宿主版本头、被拒时的处置建议都由它封装，响应体走 stdout，诊断行走 stderr，可直接接 `jq`：
...
# 没给密钥来源时它也发一次请求, 专门读宿主自报的版本头
go run scripts/management-api.go -base http://<host>:8317
```

密钥来源优先级 `-key-file` > `-key-name`（读 `~/.cache/cpa-plugins/keys/<名字>`）> `-key-cmd` > `-key-env` > 沙箱目录。密钥只留在进程内，不进对话、不进证据、不进 shell 历史。取不到或不被接受就停下来问用户，不要改试别的凭据；用户已在浏览器登录管理面时还有最后一个来源，用 CDP 把页面里的值重定向进密钥文件，不打印。

**放法用原值，不加 `Bearer`。** 宿主两种都收，`Authorization: <key>` 原值或 `X-Management-Key: <key>`，脚本默认第一种，要换用 `-auth x-management-key`。前缀不是决定项：宿主先剥 `Bearer ` 再比，同一个值两种写法等价，去掉它只是让排查少一个变量。

**别在同一把密钥上连试。** 宿主对连续失败尝试按 IP 封禁，实测 5 次失败锁 30 分钟。`invalid management key` 只说明这把值不属于该实例，重试改变不了结论，只会把后面的判断窗口一起封掉。被拒就换来源或者停下来问人。

**脚本默认直连**，不读环境里的 `HTTP_PROXY` / `HTTPS_PROXY`，内网地址被代理吃掉会表现为超时或错误页面，确需代理时用 `-proxy <url>` 显式打开。手搓命令时还要记住脚本外没有共享 shell 变量：上一轮定义的 `$K` 在下一轮是空的，空令牌换来的 401 长得像「密钥错」，下判断前先确认取值非空。

**证据要脱敏。** 落盘前裁掉 `Authorization`、token、邮箱、手机号、完整账号标识、余额绝对值，只留方法、路径、状态码、能力字段、数量与比例。要留整份响应就先过字段裁剪，别整包 `tee`，也别开 `set -x`。
...
管理面就是驱动面，统一用 `go run scripts/management-api.go` 驱动：`-sandbox <id>` 连沙箱，`-base <url>` 配 `-key-*` 连别处，写请求用 `-method POST -body '<json>'`。逐条命令与预期结果在对应特性文件里，别即兴发挥。

| 端点 | 看什么 |
| :--- | :--- |
| `GET /v0/management/plugins` | 装载与能力声明，含 `config_fields`。最省事的 Doctor |
| `GET /v0/management/plugins/<id>/config` | 该插件在 `plugins.configs.<id>` 下的实际配置，如 `{"enabled":true}` |
| `GET /v0/management/<provider>-auth-url` | 发起登录，返回 `{status, url, state}`，配 `GET /v0/management/get-auth-status?state=<state>` 轮询，得 `wait` / `ok` / `error` |
| `GET /v0/management/auth-files` | 已落盘的凭据，沙箱里恒为空 |
| `GET /v0/management/plugin-store` | 商店视角的插件与来源 |
| `GET /v0/management/config` | 宿主运行配置，看代理、日志开关 |
| `GET /v0/management/logs` | 宿主日志文件，仅 `logging-to-file: true` 时可用，否则 400 报 `logging to file disabled` |
| `quota/providers`、`quota/fetch`、`quota/reset` | 额度，需 `v7.2.159` 及以上宿主 |

`config_fields` 为空数组时管理面板显示「该插件没有声明可视化配置字段」。面板只认这份声明，插件 `config.go` 里认多少键与它无关，两边都要对。字段类型与形状见 `sdk/pluginapi/types.go` 的 `ConfigField`。

没有「测试模型」这类端点。真实对话只能走 `POST /v1/chat/completions`，用客户端 API key，管理密钥不能替代。

同一端口只能有一个实例。要并行验两个插件就显式换 `-port`，不要双驱同一个宿主。

## Evidence

证据是命令原文加真实输出，不是「应该可以」。证据标准：

- 单测、编译通过、「日志里没报错」都不是生产端证据，生产端证据来自真实宿主进程或真实产物
- 用户能看见的行为就用用户看见的方式证明，不要用只存在于测试里的入口替代真实入口
- 跳过就是跳过。某个入口因为缺前置条件没跑到，直说它没跑到，不要用另一条路的结果冒充
- 生产实例上的日志走 `GET /v0/management/logs`（需 `logging-to-file: true`）。关掉文件日志时日志只在 stdout，归运维侧的容器日志或日志聚合管，agent 读不到就直说读不到，不要拿别的证据顶上
...
- 起过登录会话就用 `go run scripts/management-api.go -sandbox <id> -method DELETE -path '/v0/management/oauth-session?state=<state>'` 取消，否则它会一直挂在宿主的会话表里
